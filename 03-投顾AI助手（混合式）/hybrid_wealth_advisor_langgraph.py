...
# 设置API密钥 - 从环境变量读取
DASHSCOPE_API_KEY = os.getenv('DASHSCOPE_API_KEY', 'sk-xxxxxxxxx')
...
def reactive_processing(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        # 检查是否需要查询上证指数
        if "上证" in user_query or "指数" in user_query:
            logger.info("调用工具: 上证指数查询")
            tool_response = tools["上证指数查询"]["func"]("")
            print(f"[DEBUG] 工具调用结果: {tool_response}")
...
"""
        ...
        input_data = {
            ...
            "tool_info": f"工具调用结果: {tool_response}" if tool_response else "无工具调用"
        }
