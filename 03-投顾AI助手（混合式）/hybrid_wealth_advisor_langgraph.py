...
class WealthAdvisorState(TypedDict):
    ...
    # 对话记忆
    conversation_history: Optional[List[Dict[str, str]]]  # 历史对话记录 [{"role": "user/assistant", "content": "..."}]
...
def query_shanghai_index(_: str = "") -> str:
    """
    上证指数实时查询工具
    优先使用新浪财经API获取真实数据，失败时降级为模拟数据
    :param _: 忽略参数
    :return: 行情信息字符串
    """
    # 尝试获取真实数据
    data = _fetch_realtime_index("sh000001", "上证指数")
    if data:
        return f"{data['name']} 当前点位: {data['price']}，涨跌: {data['change']}，涨跌幅: {data['pct']}%"

    # 降级为模拟数据
    logger.info("上证指数实时数据获取失败，使用模拟数据")
    return "上证指数 当前点位: 3125.62，涨跌: 6.32，涨跌幅: 0.20%（模拟数据，实时数据暂不可用）"


def query_shenzhen_index(_: str = "") -> str:
    """
    深证成指实时查询工具
    优先使用新浪财经API获取真实数据，失败时降级为模拟数据
    :param _: 忽略参数
    :return: 行情信息字符串
    """
    data = _fetch_realtime_index("sz399001", "深证成指")
    if data:
        return f"{data['name']} 当前点位: {data['price']}，涨跌: {data['change']}，涨跌幅: {data['pct']}%"

    logger.info("深证成指实时数据获取失败，使用模拟数据")
    return "深证成指 当前点位: 9856.78，涨跌: 12.45，涨跌幅: 0.13%（模拟数据，实时数据暂不可用）"


def query_csi300_index(_: str = "") -> str:
    """
    沪深300实时查询工具
    优先使用新浪财经API获取真实数据，失败时降级为模拟数据
    :param _: 忽略参数
    :return: 行情信息字符串
    """
    data = _fetch_realtime_index("sh000300", "沪深300")
    if data:
        return f"{data['name']} 当前点位: {data['price']}，涨跌: {data['change']}，涨跌幅: {data['pct']}%"

    logger.info("沪深300实时数据获取失败，使用模拟数据")
    return "沪深300 当前点位: 3654.21，涨跌: 8.76，涨跌幅: 0.24%（模拟数据，实时数据暂不可用）"
...
def reactive_processing(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        tools = {
            ...
            "沪深300查询": {
                "func": query_csi300_index,
                "description": "用于查询沪深300的最新行情",
                "keywords": ["沪深300", "300"]
            }
        }

        # 简单工具调用逻辑：检查用户查询是否需要工具
        user_query = state["user_query"]
        tool_responses = []
        ...
        if not tool_responses and "指数" in user_query:
            logger.info("调用工具: 上证指数查询（默认）")
            result = query_shanghai_index("")
            tool_responses.append(result)

        tool_info_str = "\n".join(tool_responses) if tool_responses else "无工具调用"

        # 调用LLM生成响应，结合工具结果
        prompt_template = """你是一个财富管理投顾AI助手。请针对用户的查询提供直接的回答。
...
"""
        ...
        input_data = {
            ...
            "tool_info": f"工具调用结果:\n{tool_info_str}" if tool_responses else "无工具调用"
        }
        ...
        return {
            **state,
            "final_response": result.get("direct_answer", "无法提供响应"),
            "emergency_response": result
        }
    except Exception as e:
        logger.error(f"反应式处理出错: {str(e)}")
        return {
            **state,
...
# 创建智能体工作流
def create_wealth_advisor_workflow(use_memory: bool = False) -> StateGraph:
    """
    创建财富顾问混合智能体工作流
    :param use_memory: 是否启用对话记忆（MemorySaver checkpoint）
    :return: 编译后的工作流
    """
    # 创建状态图
    ...
    if use_memory:
        try:
            ...
            logger.warning("langgraph.checkpoint.memory 不可用，跳过对话记忆功能")
...
# 运行智能体
def run_wealth_advisor(user_query: str, customer_id: str = "customer1",
                       thread_id: Optional[str] = None) -> Dict[str, Any]:
    """
    运行财富顾问智能体并返回结果
    :param user_query: 用户查询文本
    :param customer_id: 客户ID
    :param thread_id: 对话线程ID，传入此参数启用多轮对话记忆
    :return: 智能体运行结果
    """
    # 是否启用记忆
    use_memory = thread_id is not None
    agent = create_wealth_advisor_workflow(use_memory=use_memory)
...
async def arun_wealth_advisor(user_query: str, customer_id: str = "customer1",
                               thread_id: Optional[str] = None) -> Dict[str, Any]:
    """
    运行财富顾问智能体并返回结果（异步版本）
    :param user_query: 用户查询文本
    ...
    """
    import asyncio
    return await asyncio.to_thread(run_wealth_advisor, user_query, customer_id, thread_id)
